...
public class GolemBehvour : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		//m_Target = GameObject.FindWithTag ("Player").GetComponent<Rigidbody2D> ().position; // Add This V
		if (Vector2.Distance (m_BossBody.transform.position, m_target.position) > 0.5f) 
		{
			...
			CurrentPos = Vector3.MoveTowards (CurrentPos, m_target.position, m_speed * Time.deltaTime); //Move towards the player
